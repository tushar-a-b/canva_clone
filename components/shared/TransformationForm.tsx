...
const TransformationForm = ({ action, data = null, userId, type, creditBalance, config = null }: TransformationFormProps) => {
    ...
    const onTransformHandler = async () => {
        ...
        setIsTransformationConfig(deepMergeObjects(newTransformation, transformationConfig))
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                    ...
                    {type === 'fill' && (
                        <CustomField
                            ...
                            formLabel="Aspect Ratio" className="w-full"
                            render={({ field }) => (
                                <Select onValueChange={(value) => onSelectHandler(value, field.onChange)} value={field.value}>
                                    <SelectTrigger className="select-field">
                                        <SelectValue placeholder="Select Size" />
                                    </SelectTrigger>
                                    <SelectContent>
                                        {Object.keys(aspectRatioOptions).map((key, i) => (
                                            <SelectItem key={i} value={key} className="select-item">{aspectRatioOptions[key as AspectRatioKey].label}</SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                            )}
                    ...
                    {(type === 'remove' || type === 'recolor') && (
                        <div className="prompt-field">
                            ...
                            />
                            {type === 'recolor' &&
                                <CustomField control={form.control} name="color" formLabel="Replacement Color" className="w-full"
                                    render={({ field }) => (
